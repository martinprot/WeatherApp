...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    let dataStore: CoreDataManager? = try? CoreDataManager(modelName: "MeteoApp")


    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {

        // prevents main UI to load on unit tests
        ...
        guard let dataStore = self.dataStore else {
            fatalError("Unable to create or load data store")
        }

        // initialization delegate
        self.window = UIWindow(frame: UIScreen.main.bounds)
        let appCoordinator = AppCoordinator(dataManager: dataStore)
    ...
    // MARK: return true if XCTest are disabled
